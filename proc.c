...
int getslice(int pid){
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state != UNUSED && p->pid == pid){
      int currSliceStore = p->timeslice;
      release(&ptable.lock);
      return currSliceStore;
    }
  }
  release(&ptable.lock);
  return -1;
...
int fork2(int slice){
cprintf("%s","Inside fork2\n");
 if (slice > 0) { //different from what ta told
    int i, pid;
    struct proc *np;
    struct proc *curproc = myproc();
